...
import 'service_view_all_screen.dart';

class ServiceCategoriesScreen extends GetView<CategoryContoller> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppbar(
        /// item name
        title: StaticString.serviceCategoryTitle,
      ),
  ...
  GridView _buildCategoryList() {
    return GridView.builder(
      ...
      itemCount: controller.getCategoryServiceList.length,
      itemBuilder: (BuildContext ctx, index) {
        return GestureDetector(
          onTap: () async {
            Get.to(
              () => ServiceViewAllScreen(
                catId: controller.getCategoryServiceList[index].id.toString(),
                searchedTextName: searchedText,
              ),
            );
          },
          child: CategoryCard(
            categoryModel: controller.getCategoryServiceList[index],
          ),
        );
      },
    );
